...
const PROTOTYPE_JS_EXTERNAL_URLS = [
...
async function analyzeMarketplace(inputUrl) {
  ...
  const jsExternalUrls = [...PROTOTYPE_JS_EXTERNAL_URLS];
...
function ensurePrototypeNavigation(carouselContainer) {
  const existingNavigation = carouselContainer.next(".cp-slick-navigation");
  ...
  carouselContainer.after(
    `<div class="cp-slick-navigation" data-prototype-nav="true">
      <div class="cp-slick-arrows" data-prototype-arrows="true"></div>
      <div class="cp-slick-dots" data-prototype-dots="true"></div>
    </div>`,
  );
}
...
.cp-banner-prototype .cp-slick-navigation {
  display: flex;
  align-items: center;
...
.cp-banner-prototype .cp-slick-arrows {
  display: flex;
  align-items: center;
  gap: 10px;
}
...
.cp-banner-prototype .cp-slick-dots {
  display: flex;
  align-items: center;
  gap: 10px;
  margin: 0;
...
.cp-banner-prototype .slick-prev,
.cp-banner-prototype .slick-next,
.cp-banner-prototype .cp-proof-arrow,
.cp-banner-prototype .cp-proof-dot,
.cp-banner-prototype .cp-slick-dots button {
  border: 0;
  padding: 0;
  font: inherit;
  cursor: pointer;
...
.cp-banner-prototype .slick-prev::before,
.cp-banner-prototype .slick-next::before {
  display: none;
}

.cp-banner-prototype .cp-slick-dots .slick-dots,
.cp-banner-prototype .cp-slick-dots,
.cp-banner-prototype .cp-proof-dots {
  padding: 8px 12px;
  border-radius: 999px;
...
.cp-banner-prototype .slick-dots li,
.cp-banner-prototype .cp-proof-dots button {
  margin: 0;
}

.cp-banner-prototype .slick-dots button,
.cp-banner-prototype .cp-proof-dot {
  width: 10px;
  height: 10px;
  border-radius: 999px;
...
.cp-banner-prototype .slick-dots button::before {
  display: none;
}

.cp-banner-prototype .slick-dots li.slick-active button,
...
function buildPrototypeJs() {
  ...
  const arrowsHost = nav?.querySelector("[data-prototype-arrows='true']");
  const dotsHost = nav?.querySelector("[data-prototype-dots='true']");
  ...
  const initializeSlick = () => {
    ...
    slides.forEach((slide) => {
      ...
      slide.removeAttribute("data-prototype-active");
    });

    $slider.on("init reInit afterChange", (event, slick, currentSlide) => {
    ...
    $slider.slick({
      slide: "[data-prototype-slide]",
      slidesToShow: 1,
      slidesToScroll: 1,
      infinite: true,
      autoplay: false,
      dots: Boolean(dotsHost),
      arrows: true,
      appendDots: dotsHost ? window.jQuery(dotsHost) : window.jQuery(slider),
      appendArrows: arrowsHost ? window.jQuery(arrowsHost) : window.jQuery(slider),
  ...
  fallbackNav.className = "cp-proof-nav";
  ...
  fallbackNav.append(previousButton, dots, nextButton);

  if (nav) {
    nav.replaceWith(fallbackNav);
  } else {
...
function buildCodepenPayload({ title, description, html, css, js, cssExternal, jsExternal }) {
