...
async function analyzeMarketplace(inputUrl) {
  ...
  const headFontCss = collectHeadFontCss($, sourceUrl);
  ...
  const prototypeCss = buildPrototypeCss(headFontCss);
  ...
  if (headFontCss) {
    notes.push("Detected inline font imports in the page head and added them to the CSS panel.");
  }
...
function collectHeadFontCss($, sourceUrl) {
  const snippets = [];

  $("head style").each((_, element) => {
    const cssText = String($(element).html() || "").trim();

    if (!cssText) {
      return;
    }

    if (!/@font-face\b/i.test(cssText) && !/@import\b/i.test(cssText)) {
      return;
    }

    snippets.push(normalizeHeadFontCss(cssText, sourceUrl));
  });

  return snippets.join("\n\n").trim();
}

function normalizeHeadFontCss(cssText, sourceUrl) {
  return absolutizeCssImportUrls(absolutizeStyleUrls(cssText, sourceUrl), sourceUrl);
}
...
function prepareHeroMarkup($, element, sourceUrl, notes) {
  ...
  if (bestSlideSelector) {
    ...
    if (carouselContainer?.length) {
      ...
      const navigationAnchor = findPrototypeNavigationAnchor(carouselContainer);
      ensurePrototypeNavigation(navigationAnchor);
    }

    slides.each((index, slide) => {
      const currentSlide = fragment(slide);
      unwrapFullSlideAnchor(fragment, currentSlide);
      stripAnimatedClasses(fragment, currentSlide);
...
function findPrototypeNavigationAnchor(carouselContainer) {
  const heroSection = carouselContainer.closest("section.heroCarousel, section.hero-carousel, section[class*='hero']");
...
function ensurePrototypeNavigation(navigationAnchor) {
  const existingNavigation = navigationAnchor.next(".cp-proof-navigation");

  if (existingNavigation.length) {
    return;
  }

  navigationAnchor.after(
...
function stripAnimatedClasses(fragment, slide) {
  [slide, ...slide.find("[class]").toArray().map((node) => fragment(node))].forEach((node) => {
    ...
    const nextClasses = className
      .split(/\s+/)
      .filter(Boolean)
      .filter((token) => !/^animated$/i.test(token));

    if (nextClasses.length) {
      node.attr("class", nextClasses.join(" "));
      return;
    }
...
function absolutizeCssImportUrls(styleValue, sourceUrl) {
  if (!styleValue) {
    return "";
  }

  return styleValue.replace(/@import\s+(?!url\()(['"])(.*?)\1/gi, (_, quote, assetUrl) => {
    const absolute = absolutizeUrl(assetUrl, sourceUrl);
...
function buildPrototypeCss(headFontCss = "") {
  const sections = [];

  if (headFontCss) {
    sections.push(`/* Extracted head font CSS */\n${headFontCss.trim()}`);
  }

  sections.push(`/* Local prototype helpers */
html {
  background: #fff;
}
