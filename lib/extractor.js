...
function prepareHeroMarkup($, element, sourceUrl, notes) {
  ...
  fragment("[data-slick]").each((_, node) => {
    const dataSlick = fragment(node).attr("data-slick");

    if (!dataSlick) {
      return;
    }

    const updated = disableAutoplayInDataSlick(dataSlick);
    fragment(node).attr("data-slick", updated);
  });
...
function unwrapFullSlideAnchor(fragment, slide) {
  ...
  const directChildren = slide.contents().toArray().filter((node) => {
    if (node.type === "text") {
...
function disableAutoplayInDataSlick(dataSlick) {
  try {
    const parsed = JSON.parse(dataSlick);
    parsed.autoplay = false;
    parsed.autoplaySpeed = 999999;
    return JSON.stringify(parsed);
  } catch {
    return dataSlick
      .replace(/"autoplay"\s*:\s*true/gi, '"autoplay":false')
      .replace(/'autoplay'\s*:\s*true/gi, '"autoplay":false');
  }
}
...
function buildPrototypeJs() {
  ...
  const slider = root.querySelector("[data-slick]");
  if (slider) {
    const raw = slider.getAttribute("data-slick");
    ...
    if (raw) {
      try {
        ...
        parsed.autoplay = false;
        parsed.autoplaySpeed = 999999;
        slider.setAttribute("data-slick", JSON.stringify(parsed));
      } catch (error) {
        slider.setAttribute(
          "data-slick",
          raw.replace(/"autoplay"\\s*:\\s*true/gi, '"autoplay":false')
        );
      }
    }
  }

  const slides = root.querySelectorAll(".slick-slide, .heroCarousel-slide, .carousel-item, .slide, .hero-slide, .banner-slide");
